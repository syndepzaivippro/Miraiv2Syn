  ...
  if (senderID == global.data.botID) return;
    ...
    if(event.type == "message_unsend") {
     ...
     let name = await Users.getNameUser(senderID);
      if(getMsg.attachment[0] == undefined) return api.sendMessage(`😼Chú cún mang tên ${name} đã gỡ 1 tin nhắn tuổi gì đòi xóa (muốn xóa tin nhắn này thì dùng .unsend)\nNội dung: ${getMsg.msgBody}`,threadID)
      else {
            let num = 0
            let msg = {
              body:`😼Chú cún mang tên ${name} vừa gỡ ${getMsg.attachment.length} tệp đính kèm (muốn xóa tin nhắn này thì dùng .unsend).${(getMsg.msgBody != "") ? `\n\nNội dung: ${getMsg.msgBody}` : ""}`,
              attachment:[],
